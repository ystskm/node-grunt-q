...
function WorkerGroup(num, callback) {
  ...
  var execArgv = [].concat(process.execArgv).map(function(v) {
    return /--debug/.test(v) ? '--debug': v;
  });
...
function eventBinding() {
  ...
  function taskRun(task_id, task_read) {
    ...
    util.log('Task package"' + pkg_json + '" start on PID#' + pid + '.');
    console.log(task_read.workdir, pkg_json, task_read.cmd)
    gr.run(task_read.workdir, pkg, task_read.cmd).on('start', onStart).on(
