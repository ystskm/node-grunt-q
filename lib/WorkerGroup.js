...
function eventBinding() {
  ...
  function taskRun(task_id, task_read) {
    ...
    gr.run(task_read.workdir, pkg_json, task_read.cmd).on('start', onStart)
      .on('error', onError).on('data', onData).on('finish', onFinish)
      .on('end', onEnd);
    ...
    function onError(e, task) {
      if(e instanceof Error) return send(task_id, 'error', [e.message, task,
        e.stack]);
